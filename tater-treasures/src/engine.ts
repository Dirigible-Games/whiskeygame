...
export function getAgeDisplay(bottle: { age: number, region: Region, modifiers: string[] }): string {
  ...
  if ([Region.SCOTLAND, Region.IRELAND, Region.JAPAN].includes(bottle.region)) {
    if (bottle.age < 10) {
      return 'Unstated';
    }
  }
...
export class WhiskeyEngine {
  ...
  private generateWorld() {
    // Heavily weight Scotland to compensate for their smaller brand portfolios, ensuring a massive world
    const regions = [Region.USA, Region.USA, Region.USA, Region.SCOTLAND, Region.SCOTLAND, Region.SCOTLAND, Region.SCOTLAND, Region.IRELAND, Region.CANADA, Region.JAPAN];
    
    // Generate 80 distilleries to create a massive world
    for (let i = 0; i < 80; i++) {
      ...
      if (region === Region.SCOTLAND) {
        // Scottish distilleries have very few brands, even the legacy ones
        if (distillery.category === DistilleryCategory.LEGACY) {
          brandCount = Math.floor(Math.random() * 3) + 1; // 1-3 brands
        } else if (distillery.category === DistilleryCategory.ESTABLISHED) {
          brandCount = Math.floor(Math.random() * 2) + 1; // 1-2 brands
        } else if (distillery.category === DistilleryCategory.DEFUNCT) {
          brandCount = Math.floor(Math.random() * 2) + 1; // 1-2 brands
        } else {
          // YOUNG
          brandCount = 1;
        ...
        // USA, Ireland, Canada, Japan act as mega-distilleries or corporate houses
        if (distillery.category === DistilleryCategory.LEGACY) {
          brandCount = Math.floor(Math.random() * 10) + 6; // 6-15 brands
        } else if (distillery.category === DistilleryCategory.ESTABLISHED) {
          brandCount = Math.floor(Math.random() * 4) + 2; // 2-5 brands
        } else if (distillery.category === DistilleryCategory.DEFUNCT) {
          brandCount = Math.floor(Math.random() * 4) + 1; // 1-4 brands
        } else {
          // YOUNG
          brandCount = Math.floor(Math.random() * 2) + 1; // 1-2 brands
          useDistilleryName = true;
  ...
  private generateProductLinesForBrand(brand: Brand, distillery: ParentDistillery) {
    ...
    if (region === Region.SCOTLAND || region === Region.IRELAND || region === Region.JAPAN) {
      ...
      const count = category === DistilleryCategory.LEGACY ? 7 : category === DistilleryCategory.ESTABLISHED ? 4 : 2;
      
      let brandModifiers: string[] = [];
      let currentType = type;
      
      if (region === Region.SCOTLAND) {
        if (Math.random() < 0.25) {
          brandModifiers.push('Peated');
        }
        if (Math.random() < 0.3) {
          currentType = WhiskeyType.BLENDED_SCOTCH;
        }
      }
      ...
      if (category === DistilleryCategory.LEGACY) {
        ...
        for (let i = 0; i < specialCount; i++) {
          this.productLines.push(this.createProductLine(
            ...
            12 + Math.floor(Math.random() * 12), // Up to 23 years old
  ...
  private generateDistillery(region: Region): ParentDistillery {
    // Distribution: 15% Defunct, 40% Young, 30% Established, 15% Legacy
    const rand = Math.random();
    let category: DistilleryCategory;
    if (rand < 0.15) category = DistilleryCategory.DEFUNCT;
    else if (rand < 0.55) category = DistilleryCategory.YOUNG;
    else if (rand < 0.85) category = DistilleryCategory.ESTABLISHED;
